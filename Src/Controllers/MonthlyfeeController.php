...
use Src\Models\MonthlyfeeModel;
...
class MonthlyfeeController
{
   ...
   /* private function ValidaInput($response,$dados) : bool
    ...
    {   
        ...
        {   
            $parm['qtde_registros'] = 12;

            /*
            if(isset($header->data['qtde_registros']))
            {
                $parm['qtde_registros'] = addslashes($header->data['qtde_registros']);
            }    
            */ 

            return $this->model->Buscar($parm,$response,$body);
            die();
        }
